...
export default {
  ...
  methods: {
    ...
    info(id) {
      this.$http({
        url: `discusswenzhangfenxiang/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `discusswenzhangfenxiang/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.discusswenzhangfenxiangCrossAddOrUpdateFlag = false;
						 this.parent.search();
